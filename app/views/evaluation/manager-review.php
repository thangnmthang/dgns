...
$submissionUrl = $isManager ? $GLOBALS['config']['base_url'] . 'lanh-dao-danh-gia' : $GLOBALS['config']['base_url'] . 'form-danh-gia';
...
<div class="bg-white rounded-lg shadow-md p-6 print:shadow-none print:p-0">
    ...
    <?php if ($isNewForm): ?>
        ...
        <?php elseif ($isEvaluator && ! $hasEvaluated && ! $isNewForm): ?>
            <form method="POST" action="<?php echo $reviewType === 'manager' ? $GLOBALS['config']['base_url'] . 'lanh-dao-review/' . $evaluation['id'] : $GLOBALS['config']['base_url'] . 'admin/evaluations/save-review/' . $evaluation['id'] ?>" class="space-y-6">
                <input type="hidden" name="review_type" value="<?php echo $reviewType ?>">
                <input type="hidden" name="action" value="approve">
            <?php endif; ?>

            <?php if ($isNewForm && isset($userDepartments) && count($userDepartments) > 0): ?>
            ...
            <div class="<?php echo $isNewForm ? '' : 'print:block' ?>">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        ...
                        $totalSelfScore  = 0;
                        // $totalRescore    = 0;
                        $rescore         = (isset($reviewType) && $reviewType === 'manager') ? $managerScore : ((isset($reviewType) && $reviewType === 'deputy_director') ? $deputyDirectorScore : $directorScore);
                        ...
                        ?>
                                <input type="hidden" name="rescore[<?php echo $criteriaId ?>]" value="<?php echo $score ?>">
                            <?php
                            }
                        }
                                    ...
                                    <?php else: ?>
                                        ...
                                        <td class="border border-gray-300 p-2 text-center">
                                            ...
                                            <?php else: ?>
                                                ...
                                                $leaderrescore = isset($evaluation['leader_rescore']) ? json_decode($evaluation['leader_rescore'], true) : [];
                        ...
                        <tr class="bg-blue-50">
                            ...
                            <?php if ($isNewForm): ?>
                                <td class="border border-gray-300 p-2 text-center font-bold"><span id="rescore-total"><?php echo $totalRescore ?></span></td>
                            <?php else: ?>
                                <td class="border border-gray-300 p-2 text-center font-bold"><span ><?php echo $evaluation['leader_rescore_total'] ?? 0 ?></span></td>
                            <?php endif; ?>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <?php if ($isNewForm): ?>
                                    ...
                                    <?php else: ?>
                                        <?php echo $evaluation['extra_deduction_leader'] ?? 0 ?>
                                    <?php endif; ?>
                            </td>
                            <!-- <td colspan="4"></td> -->
                        </tr>
                        <tr>
                            ...
                            <td class="border border-gray-300 p-2 text-center font-bold">
                                <span><?php echo $evaluation['leader_rescore_final'] ?? 0 ?></span>
                            </td>
                           
                            <td aria-controls="" colspan="4"></td>
                        </tr>
                        <?php
                        $finalScore = ($totalRescore > 0 ? $totalRescore : $totalSelfScore) - ($evaluation['extra_deduction_leader'] ?? 0);
                        $dutyScore = 0;
                ...
                <div class="space-y-4">
                    ...
                    <div class="mt-6">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Nhập ghi chú nếu cần..."></textarea>
                    </div>
                    ...
                    <!-- Bổ sung điều kiện $isNewForm -->
                <?php elseif ($isEvaluator && ! $hasEvaluated): ?>
                    <div class="mt-6">
                        <label for="manager_comment" class="block text-sm font-medium text-gray-700 mb-1">Nhận xét đánh giá</label>
                        <textarea id="manager_comment" name="manager_comment" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Nhập nhận xét chi tiết của bạn..." required></textarea>
                    </div>
                    <div class="mt-6 flex justify-end">
                ...
                <div class="mt-6 grid grid-cols-3 gap-4<?php echo $isNewForm ? 'hidden' : '' ?>">
                    <div>
                        ...
                        <p class="min-h-[80px] border-b border-gray-300 py-2"><?php echo isset($evaluation['manager_comment']) ? $evaluation['manager_comment'] : '' ?></p>
                    </div>
                    <div>
                        <p class="font-bold mb-2">Ý kiến nhận xét của phó giám đốc:</p>
                        <p class="min-h-[80px] border-b border-gray-300 py-2"><?php echo isset($evaluation['deputy_director_comment']) ? $evaluation['deputy_director_comment'] : '' ?></p>
                    </div>
                    <div>
                        <p class="font-bold mb-2">Ý kiến nhận xét của giám đốc:</p>
                        <p class="min-h-[80px] border-b border-gray-300 py-2"><?php echo isset($evaluation['director_comment']) ? $evaluation['director_comment'] : '' ?></p>
                    </div>
                </div>
            </div>
        ...
        <?php if ($userRole === 'lanh_dao' && $isNewForm && ($evaluationStatus === 'sent' || $evaluationStatus === 'reviewed')): ?>
            <div class="mt-8 print:hidden">
                <h3 class="text-xl font-semibold mb-4">
                    <?php echo ! $hasEvaluated ? 'Nhập đánh giá (Lãnh đạo phòng)' : 'Chỉnh sửa đánh giá (Lãnh đạo phòng)' ?>
                </h3>
                <form method="POST" action="<?php echo $GLOBALS['config']['base_url'] ?>lanh-dao-review/<?php echo $evaluation['id'] ?>">
                    ...
                    <?php
                    // Include rescore values as hidden inputs in the second form
                    if (isset($rescore) && is_array($rescore)) {
                        foreach ($sections as $section) {
                            foreach ($section['criteria'] as $criteria) {
                                $criteriaId = $criteria['id'];
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm mb-6">
                        ...
                        <div class="p-4">
                            <textarea id="manager_comment" name="manager_comment" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Nhập nhận xét chi tiết của bạn..." required><?php echo isset($evaluation['manager_comment']) ? $evaluation['manager_comment'] : '' ?></textarea>
                        </div>
                    </div>
                    <div class="flex justify-between">
                        ...
                        <?php endif; ?>
                        <button type="submit" class="px-6 py-3 bg-indigo-600 text-white font-medium rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors">
                            <?php echo ! $hasEvaluated ? 'Duyệt đánh giá' : 'Cập nhật nhận xét' ?>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <table class="min-w-full bg-white border border-gray-300">
                ...
                <tbody>
                    <?php foreach ($evaluations as $eval): ?>
                        ...
                        ?>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300"><?php echo date('d/m/Y H:i', strtotime($eval['created_at'])) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300 font-bold"><?php echo $evalTotalScore ?> / 100</td>
